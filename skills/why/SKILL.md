...
metadata:
  origin: pstack `why` (github.com/cursor/plugins, MIT); references verbatim; harness-specific mechanics (Cursor subagent config, model labels, MCP discovery) replaced with harness-neutral wording on 2026-09-19
---
...
Before spawning investigators, list the MCP servers and tools your harness exposes in this session (its tool list, its MCP configuration, or its `mcp list` command).
...
Subagent config (each), using your harness's subagent facility:
- Type: a general-purpose agent with tool access.
- Model: your configured why-investigators model (the harness's default subagent model unless the user names one; a fast model suits this role).
- Mode: agent mode with tools, **not** a read-only or ask mode that strips MCP access, which disables MCP-backed investigators entirely. Investigators still shouldn't write anything; say so in the prompt.
- If your harness has no subagent facility, run the investigator roles yourself, one source at a time, and label each block of findings with its source.
...
Spawn one synthesizer subagent, using your harness's subagent facility:

- Type: a general-purpose agent with tool access.
- Model: your configured why-synthesizer model (the harness's default subagent model unless the user names one; this is the role for a stronger reasoning model when one is offered).
- Mode: agent mode with tools. The synthesizer's quality check spot-verifies citations, which can require MCP access; a read-only or ask mode that strips MCPs defeats that. It must not write files, commit, or modify external state.
- If your harness has no subagent facility, do the synthesis yourself from the collected findings, following the same template.
